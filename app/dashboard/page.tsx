...
export default async function DashboardPage() {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/onboarding" className={buttonVariants()}>Create your group</Link>
  ...
  const [totals, pendingFines, nextMeeting, group, mgrNext, welfareFund] = await Promise.all([
    ...
    products.mgr ? withTenant(groupId, async (tx) => {
      ...
      return computeNextMgrEvent(cycles.map((c) => ({ cycleNumber: c.cycleNumber, status: c.status, scheduledDate: c.scheduledDate })), slots.map((s) => ({ cycleNumber: s.cycleNumber, memberId: s.memberId, memberName: s.member?.name ?? null })), new Date());
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-semibold tracking-tight">Welcome back, {session.user.name}</h1><p className="text-sm text-muted-foreground">{groupName} · <span className="capitalize">{groupType}</span> · <span className="capitalize">{role}</span></p></div>
      {isStaff && group && !group.registrationComplete && <Card className="border-amber-500/50 bg-amber-500/5"><CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6"><p className="text-sm">This group&apos;s registration isn&apos;t complete — it needs a Treasurer and Secretary assigned before it&apos;s publicly discoverable or can approve new members.</p><Link href="/members" className={buttonVariants({ size: "sm", variant: "outline" })}>Assign officials</Link></CardContent></Card>}
      <div className={`grid gap-4 sm:grid-cols-2 ${showWelfare ? "lg:grid-cols-5" : "lg:grid-cols-4"}`}><Metric label="Members" value={String(totals.memberCount)} /><Metric label="Capital" value={ksh(totals.capital)} /><Metric label="Security" value={ksh(totals.security)} /><Metric label="Personal savings" value={ksh(totals.personalSavings)} />{showWelfare && welfareFund && <Metric label="Welfare fund" value={ksh(Number(welfareFund.emergencyBalance) + Number(welfareFund.longTermBalance) + Number(welfareFund.advanceBalance))} />}</div>
      {products.mgr && <Card className={mgrNext && mgrNext.daysUntil < 0 ? "border-destructive/50 bg-destructive/5" : undefined}><CardHeader className="flex flex-row items-center justify-between"><CardTitle className="text-base">Next MGR payout</CardTitle><Link href="/insights" className="text-xs text-muted-foreground underline-offset-4 hover:underline">View insights</Link></CardHeader><CardContent>{mgrNext ? <div className="text-sm"><p className="text-lg font-semibold">{mgrNext.recipientName ?? "Unassigned"}</p><p className="text-muted-foreground">Cycle {mgrNext.cycleNumber} · {mgrNext.scheduledDate} · {mgrNext.daysUntil < 0 ? `${Math.abs(mgrNext.daysUntil)} day${Math.abs(mgrNext.daysUntil) === 1 ? "" : "s"} overdue` : mgrNext.daysUntil === 0 ? "today" : `in ${mgrNext.daysUntil} day${mgrNext.daysUntil === 1 ? "" : "s"}`}</p></div> : <p className="text-sm text-muted-foreground">No MGR cycle is active or scheduled yet.</p>}</CardContent></Card>}
