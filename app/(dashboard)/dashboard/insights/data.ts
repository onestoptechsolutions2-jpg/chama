...
/**
 * Everything InsightsPage needs *except* the staff-only historical `report`
 * (monthly totals / top balances / loans-and-fines-by-status) — that stays
 * page-local since it's pure display data no recommendation reads, and
 * fetching it anywhere recommendations are just being summarized (e.g. the
 * dashboard home) would pay for three extra queries nothing there renders.
 *
 * Mirrors InsightsPage's own query shape exactly: each independent aggregate
 * gets its own `withTenant` call, all run concurrently via an outer
 * `Promise.all` — never a shared transaction, per the `Promise.all`-inside-
 * `withTenant` RLS race this codebase has hit (and fixed) twice already (see
 * docs/architecture.md's Scale Audit and Post-Guarantors sections).
 */
export async function computeGroupInsights(
  groupId: number,
  ...
  if (!core.group) {
    return { recommendations: [], nextMgrEvent: null, mgrPace: null, memberRiskFlags: [] };
  }
  const { group } = core;
  ...
  return { recommendations, nextMgrEvent, mgrPace, memberRiskFlags };
}
